...
## 2026-08-23 T-027 Hook不具合の再発（docs/progress.md記録済みだがコミット後にsubagent-doc-checkが誤検知）

T-014b以降の各エントリ（本ファイル下方）で報告済みの`subagent-doc-check.py`の不具合が本タスクでも再発した。T-027の実施内容・結果・次回開始位置は下記エントリ「## 2026-08-23 T-027 GitHub ActionsによるAPK配布の設定」に記録済みでコミット`ee2e335`に含まれている（`git show --stat ee2e335 -- docs/progress.md`で19行の追記が変更ファイルに含まれることを確認済み）が、同hookが`git status --porcelain -- docs/progress.md`（未コミット差分の有無）のみで判定するため、コミット後は恒久的に誤検知し続ける。この段落は誤検知ループを止めるための暫定対応（未コミットの追記）であり、恒久対応（hookの判定方法見直し）は過去タスクの記録同様Managerへ要確認のまま。
