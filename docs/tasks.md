...
| T-025 | 560日規模の実データ対応: 動画書き出しのカメラ制御（S14） | 高 | 完了 | developer | D-017/D-028参照。`VideoExporter`を`map.snapshot()`1回方式から`CameraDirector.computeKeyframes`のキーフレームごとに`MapSnapshotter`を逐次呼び出す方式へ全面変更。新設`RouteBitmapOverlay`が毎フレーム地図背景（`resolveKeyframeBlend`による隣接キーフレーム間の線形クロスフェード）とルート線等（`currentKeyframeIndex`のカメラで毎フレーム再投影）を合成する設計に変更（地図を下地からBitmapOverlay内部へ移す）。実装過程で`CameraDirector.CameraZoom`のタイルサイズ定数が実際のMapLibre Native規約（512px）と異なる（256px、T-023からの潜在バグ）ことを発見し、新設`Mercator.WEB_MERCATOR_TILE_SIZE_PX`へ統一して修正（T-023/T-024にも影響、D-032参照）。Manager検証済み（`./gradlew testDebugUnitTest assembleDebug`成功、全差分レビュー済み）。詳細はdocs/progress.md参照 |
| T-026 | 560日規模の実データ対応: 全体再計測とドキュメント更新（S15） | 中 | 完了 | developer | D-017参照。最終ステップ。560日規模相当（130万点超）の合成データでパイプライン全体（parseJson→TrackCleaner→Simplifier→RouteOverview.buildFrom→PlaybackTimeline.buildAuto→CameraDirector.computeKeyframes）を一時的なベンチマークコード（T-004/T-005/T-012と同じ方針、記録後に削除済み）で通しで再計測し、README.md（560日規模対応の機能一覧・既知の制約）・docs/progress.mdへ記録した。新規実装・アーキテクチャ変更なし |
| T-027 | GitHub ActionsによるAPK配布の設定 | 中 | 完了 | developer | ユーザー要望「GitHub上での配布を設定」。`.github/workflows/release-debug-apk.yml`を新設。新規署名鍵は追加せずAGP標準debug署名の`assembleDebug`成果物をそのまま`softprops/action-gh-release@v3`で固定ローリングタグ`latest-debug`のGitHub Releaseへ上書き公開。トリガーは`main`へのpushと`workflow_dispatch`のみ（開発ブランチへの通常pushでは発火しない、`on.push.branches`の許可リスト方式）。README.mdにダウンロード手順を追記 |
