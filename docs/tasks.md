...
| T-007 | アニメーション再生と速度制御 | 高 | レビュー中 | developer | データ時刻↔再生時刻の単調写像、自動速度（非線形圧縮）・手動倍率。Reviewer指摘によりT-007bで修正ループ中（D-008参照） |
| T-007b | T-007レビュー指摘の修正（再生中シークの競合、trimByProgress二重計算） | 高 | 未着手 | developer | D-008参照 |
