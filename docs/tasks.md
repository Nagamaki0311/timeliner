...
| T-012 | 560日規模の実データ対応: 計測基盤とSimplifierのANR根治（S0+S1） | 高 | 未着手 | developer | D-017参照。ユーザーが560日分の実データで再生開始時にクラッシュ。根本原因は`Simplifier`の時間ガード処理がO(n²/s)に退化しUIスレッド上で最大61回反復されること |
