...
### 独立検証による追記（2026-08-22、Manager指示によるReviewer再検証）

- 上記決定1〜7は、GitHub一次ソース（`android-v13.5.0`タグ）とプロジェクト実依存のaarバイトコード逆コンパイルの両方を使った独立した2回目の検証でも全てCONFIRMEDされた（例外メッセージ文字列のバイトコード一致、および決定5の設計方針と完全に一致する公式サンプル`MapSnapshotterReuseActivity.kt`の発見という、より強い追加根拠を得た）。
- 決定6「各`start()`呼び出しはスタイル解析・タイル取得・GLレンダリングを行うフルパイプライン」は、ネイティブコアを直接読むとやや不正確と判明した。単一の`mbgl::Map`インスタンスがSnapshotterの生存期間中保持されるため、**スタイル解析が発生するのは初回ロード時、または明示的に`setStyleUrl`/`setStyleJson`を呼んだ場合のみ**であり、カメラ位置のみを変えて`start()`を繰り返すT-023のキーフレーム方式では毎回のスタイル解析は発生しない（実際のコストは決定6の記述より低い、結論を覆す誤差ではない）。
- **T-023/T-025着手時に踏まえるべき追加制約**: `MapSnapshotter.Options.withStyleBuilder()`で指定したカスタムsource/layer/imageは、`onDidFinishLoadingStyle`内の`fullyLoaded`ガードにより**初回のスタイルロード時にしか適用されない**。同一インスタンスを再利用しつつ`setStyleUrl`で異なるスタイルへ切り替えても、カスタムsource/layerは新しいスタイルへ再適用されない。キーフレームごとに同一スタイル・カメラのみ変更する現在の設計では実害無いが、将来スタイル自体を切り替える設計に発展させる場合は注意が必要。
- ネイティブ層（`map_snapshotter.cpp`の`Impl::snapshot()`）にも再入防止ガード（`MisuseException`）があるが、この分岐はreturnせずフォールスルーし`renderStillCallback`を上書きしてしまう不完全な実装であることが判明した。通常はAndroid Kotlin側の`check()`ガード（決定5）で到達しないため実害は無いが、ネイティブ層側の防御は完全でないという事実として記録する。

---

## D-029: T-023レビュー指摘への対応方針（隣接キーフレーム窓の境界点二重カウント、日付変更線bboxの誤り）

- 日付: 2026-08-22
- 状態: 採用

### 背景
- T-023（CameraDirector、コミット`7a748af`）のレビューで、ReviewerがMedium 2件・Nit 1件を検出した。
  1. （Medium/CONFIRMED、コード実行で実証済み）`buildKeyframe`の隣接キーフレーム間の時間窓が、共有境界（データ時刻）を両側とも閉区間で扱っているため、境界に一致する点が両方の窓（bbox計算）に二重に含まれる。CameraDirectorのKDoc・docs/progress.mdの「重複・隙間なく再生時間全体を分割する」という記述と実態が矛盾している。
  2. （Medium/CONFIRMED、コード実行で実証済み）`CameraZoom.zoomToFitBounds`は経度180度（日付変更線）をまたぐbboxへの補正分岐（`longitudeDiff < 0.0`）を持つが、`GeoBounds.compute`（新規範囲指定版・既存全点版とも）は単純min/maxでラップアラウンドを考慮しないため`longitudeDiff`が負になることは実際には起こらず、この補正は到達しないデッドコードになっている。結果、日付変更線をまたぐルート（太平洋横断等）でズームレベルが大きく誤りうる（実測で同じ物理距離1度差でも通常時zoom≈10.57に対し日付変更線をまたぐ場合zoom≈2.08）。`GeoBounds`自体の日付変更線非対応はT-006/T-013から既知の制約（D-007参照）だが、あたかも対応しているかのような補正分岐を含みながら実際には機能しない点は本コミット固有の問題。
  3. （Nit/CONFIRMED、実害なし）`buildKeyframe`内で`lowerBound(timestampsMillis, dataStart)`を同じ引数で2回計算している（冗長、性能上無視できる）。

### 決定
1. 1（Medium）・3（Nit、対応コストが極めて小さいためあわせて実施）を修正する（T-023b）。境界を片側開区間にし（最終キーフレームの`windowEnd`のみ閉区間の特別扱いを維持）、重複を解消する。KDoc・docs/progress.mdの記述も実態と一致させる。冗長な`lowerBound`呼び出しは既存の`fromIndex`変数を再利用する。
2. 2（Medium、日付変更線bboxの誤り）は今回対応せず、`CameraZoom`のKDoc/コメントから「日付変更線を正しく処理している」という誤解を招く記述を外し、`GeoBounds`の既知の制約（D-007由来）に起因する未対応事項として明記した上でdocs/tasks.mdバックログへ記録するに留める。

### 理由
- 1は設計文書・実装が主張する性質と実態が食い違っており、AGENTS.md原則7（バグは根本原因を直す）・REVIEW.mdの完了条件「実装と設計が一致している」に照らし修正する。境界に一致する点は実データ（一定間隔記録）では珍しくないため実害もある。
- 2は`GeoBounds`自体の日付変更線非対応というT-006以来の既知の制約（D-007）に起因し、本タスクの範囲でこの根本対応（bboxの循環座標系対応）を行うのはスコープが大きい。560日規模の実データで日付変更線をまたぐ記録が実際に発生するかは要件次第であり、確証の無いまま優先的に対応するよりも、誤解を招く記述を正した上で既知の制約として記録し、実際にそのようなデータが問題になった時点で対応する方が合理的（AGENTS.md原則1、理解してから作る）。

### 影響
- 以降、隣接する時間窓・区間を「重複・隙間なく分割する」設計を導入する場合、境界の扱い（開区間/閉区間）を明示的に決め、実際にテストで境界点の帰属を検証するパターンを踏襲する。
- `GeoBounds`の日付変更線非対応は、以降bboxを扱う新規コード（今回のT-023、将来のT-024/T-025含む）で繰り返し踏まえるべき既知の制約として、この決定を参照する。

---

## D-030: T-023bレビュー指摘への対応方針（退化ケースのブラケット処理が境界二重カウントを再導入する）

- 日付: 2026-08-22
- 状態: 採用

### 背景
- T-023b（D-029決定1の修正、コミット`f8c4d05`）のレビューで、ReviewerがMedium 1件・Low 1件を検出した。
  1. （Medium/CONFIRMED、コード実行で実証済み）T-023bは「窓内に1点以上ある通常ケース」の境界のみを片側開区間化したが、「窓内に点が1つも無い退化ケース」のブラケット処理（`resolveWindowIndexRange`の`fromIndex >= toIndex`分岐）は無修正のままで、依然として`nextIndex = upperBound(dataEnd)`という両側閉区間相当の計算を使っている。このため退化した窓が、隣接する非退化窓が既に持つ点と重複してブラケットしてしまい、D-029で報告されたのと同じ境界二重カウントが退化ケース経由で再発する。実際に2点のみのルート（`keyframeIntervalMillis=500`で3窓に分割、中央の窓が退化）で`index0`が窓0・窓1の両方に、`index1`が窓1・窓2の両方に含まれることを実行で確認済み。この不具合自体はT-023初版（`7a748af`）から式が変わっておらず新規混入ではないが、T-023bのコミットメッセージ・KDoc・docs/progress.mdは「境界の二重カウントを解消した」と無限定に主張しており、その主張は退化ケースについて成立していない。
  2. （Low/CONFIRMED）上記1に伴い、KDoc・docs/progress.mdの「重複・取りこぼしなく分割する」という記述が退化ケースの例外を書き漏らしており不正確。

### 決定
1. 1（Medium）・2（Low、1の修正に付随するため同時に）を修正する（T-023c）。退化ケースのブラケット処理も、通常ケースで導入した半開区間の規約（非最終窓は`dataEnd`を含まない）と整合するよう修正し、退化ケース×境界共有のシナリオ（Reviewerが実際に再現した「2点のみ・3窓・中央窓が退化」のようなケース）をテストとして追加する。KDoc・docs/progress.mdの記述も実態に合わせて更新する。

### 理由
- CameraDirectorはまだT-024/T-025から呼ばれていない先行実装のため直接のユーザー影響は無いが、次のフェーズで使われ始めた時点で顕在化する既知のバグであり、AGENTS.md原則7（バグは根本原因を直す）に照らし、次のタスクに進む前に解消しておくべきと判断した。同じ境界重複問題を2度目のレビューで指摘されている以上、今回は退化ケースも含めた網羅的な修正・テストを行う。

### 影響
- 以降、「通常ケース」「退化ケース」のように処理経路が分かれるロジックを修正する場合、修正の意図（境界規約の統一等）が両方の経路に一貫して適用されているかを確認するパターンを踏襲する。

---

## D-031: T-024レビュー指摘への対応方針（currentKeyframeIndexのKDocが主張する等価性の不成立）

- 日付: 2026-08-22
- 状態: 採用

### 背景
- T-024（画面再生でのカメラ追従、コミット`b7ef8c5`）のレビューで、ReviewerがMedium 1件・Low 2件・Nit 1件を検出した。
  1. （Medium/CONFIRMED、Pythonシミュレーションで実証済み）`CameraDirector.currentKeyframeIndex`のKDocは「`computeKeyframes`内部の窓分割と等価な結果になる」と断定しているが、実際には隣接キーフレームの境界ミリ秒ちょうどで丸め方向が食い違う（`currentKeyframeIndex`の距離比較タイブレークは前のキーフレームを優先するが、`computeKeyframes`の窓所有権は境界点を後の窓に割り当てる）。`FRAME_INTERVAL_MILLIS=16ms`刻みで進む実行時の実害はほぼ無いが、KDocの主張自体が誤り。
  2. （Low/CONFIRMED）`CAMERA_FOLLOW_DURATION_MILLIS=900`固定値の設計根拠（「キーフレーム間隔5秒より十分短い」）は、Manualモード（総再生時間が任意の値）の最終区間では約18%の確率で成立しない。
  3. （Low/CONFIRMED、API存在確認済み）一時停止直後、進行中の`easeCamera`アニメーション（最大900ms）が`MapLibreMap.cancelTransitions()`未呼び出しのため完了まで継続する。
  4. （Nit/CONFIRMED）新設テストが既存の並行性検証パターン（`setRoute`/`setSpeedMode`競合時のアサーション）に`activeCameraKeyframe`の期待値を含めていない。

### 決定
1. 1（Medium）を修正する（T-024b）。タイブレークを、`playbackMillis < (keyframes[lo-1].playbackMillis + keyframes[lo].playbackMillis) / 2`という中点比較へ変更する（Reviewer提案(b)案、Pythonシミュレーションで不一致0件を確認済み）ことで、KDocの主張を実装と一致させる。ドキュメント側の記述を弱めるのではなく、実装側を真に等価にする方針を採る。
2. 2・3・4は対応せず見送る。いずれもReviewer自身が「必須ではない」と評価しており、docs/tasks.mdバックログへ記録するに留める。

### 理由
- 1はREVIEW.mdの完了条件「実装と設計が一致している」に抵触し、かつ修正コストが1行の比較式変更で完結するため対応する。KDocを弱める（不一致を許容する記述にする）よりも、既に検証済みの中点比較へ実装を揃える方が、将来この関数を他用途へ転用する際の事故を防げる。
- 2はManualモードの限定的な区間・確率でのみ発生する軽微なカクつきで、AGENTS.md判定ラダー「過度に複雑な配線をしない」とのトレードオフをReviewer自身が認めている。3も同様に視覚的違和感にとどまりデータ損失・クラッシュを伴わない。4は既存の代入が構造上原子的であることをコードで確認済みのため、テスト追加の優先度は低い。

### 影響
- 以降、「見た目は等価に見えるが独立に実装された2つのロジック」を導入する場合、実際に全ケースを網羅的に検証（本件のようなシミュレーションや境界値テスト）してから「等価である」とKDocに明記するパターンを踏襲する。

---

## D-032: CameraZoomのタイルサイズ定数の誤り（256px→512px）の修正とT-023/T-024への遡及的影響

- 日付: 2026-08-23
- 状態: 採用

### 背景
- T-025（動画書き出しのカメラ制御、コミット`d8b50d1`）の実装中、Developerが`Mercator.metersPerPixelAtZoom`を新設する過程でMapLibre NativeのGitHub一次ソース（`android-v13.5.0`タグ、`include/mbgl/util/constants.hpp`の`tileSize_D`、`include/mbgl/util/projection.hpp`の`Projection::getMetersPerPixelAtLatitude`）を確認したところ、Webメルカトルの1タイルあたりピクセル数は**512px**だった。
- 一方、T-023（コミット`7a748af`）で実装済みの`CameraDirector.CameraZoom`（bboxが収まる最小ズームを求める`zoomToFitBounds`）は`TILE_SIZE_PX = 256.0`（クラシックなOSMタイル規約）を使用しており、実際のMapLibre Nativeの規約と1ズームレベル分ずれていた。この結果、`CameraZoom`が計算するズームは実際より1レベル高く（狭く）なり、意図したbboxの一部が画面からはみ出す潜在バグだった。
- このバグはT-023・T-023b（D-029）・T-023c（D-030）と3回のレビューを経ながら一度も指摘されなかった。3回のレビューはいずれも境界値・退化ケースの網羅性（窓分割ロジック）に焦点を当てており、ズーム計算式自体の定数をMapLibre Native一次ソースと突き合わせる検証は行われていなかったため、レビュー対象外だった（境界値検証は「ロジックが正しいか」であり「係数が正しいか」ではない、異なる観点であることが今回判明した）。
- `CameraDirector.currentKeyframeIndex`はT-024（画面再生でのカメラ追従、コミット`b7ef8c5`、D-031で1回レビュー済み）から呼ばれており、`CameraZoom`のズーム値をそのまま画面の`easeCamera`へ渡している。そのため本バグはT-024の既存の動作（画面再生時の自動追従ズーム）にも遡及的に影響する。

### 決定
1. `Mercator.kt`に一次ソースを引用した`WEB_MERCATOR_TILE_SIZE_PX = 512.0`（`const val`）を新設し、`CameraZoom.TILE_SIZE_PX`をこの共有定数を参照するよう修正する（単一の真実の情報源に統一、AGENTS.md原則7「バグは根本原因を直す」）。
2. T-023/T-024を差し戻して再修正する新規タスクは立てない。修正はT-025のコミット（`d8b50d1`）に含めて完了とし、本エントリでその遡及的影響を記録するに留める。
3. `CameraZoomTest.kt`の絶対値アサーション（viewport=256pxを「1タイル」として使っていた2件）をviewport=512pxへ更新する。

### 理由
- 1はAGENTS.md原則7「バグは根本原因を直す」に照らし、ズーム計算全体（`CameraDirector`が呼ばれる画面再生・動画書き出し双方）に影響する共通定数の誤りである以上、根本原因（誤った定数）を直接修正するのが妥当。
- 2について、T-023/T-024自体のロジック（窓分割・キーフレーム選択のアルゴリズム）に変更は無く、変わるのは`CameraZoom`が使う係数のみであり、修正はT-025が新たに`Mercator.metersPerPixelAtZoom`を必要とした結果として同じコミットに自然に含まれる（地図ラスターとルート線オーバーレイの投影がズレないよう、両者が同じ512px基準を共有する必要があった）。別タスクとして切り出すより、依存関係が生じた箇所で一度に直す方がAGENTS.md判定ラダー「過度に複雑な配線をしない」に沿う。挙動の変化（画面再生時の自動追従カメラが修正前より1ズームレベル分広い範囲を映すようになる）は意図した修正の結果であり、退行ではない。
- 3回のレビューを経たコードに見つかった今回のケースは、「境界値・退化ケースの網羅性」と「外部依存ライブラリの規約定数との整合性」が独立した検証観点であることを示している。後者は今回のように新規コードが同じ定数を別の角度から必要とする機会がなければ発見されなかった可能性があり、プロセス上の教訓として記録する。
